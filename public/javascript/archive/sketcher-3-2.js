...
function setup() {
	...
	cnv.position(50,50);
	cnv.mouseWheel(function () {
		if (gridWidth + event.wheelDelta / 10 > 30 && gridWidth + event.wheelDelta / 10 < 100) {
			gridWidth += event.wheelDelta / 10; 
		}
		drawBackground();
...
function withinCanvas(pt) {
	if (pt.x < 0 || pt.x > canvasWidth || pt.y < 0 || pt.y > canvasHeight) return false
		else return true;
}
...
function mousePressed() {
	...
	console.log(current);
	if (!withinCanvas(current)) return;
