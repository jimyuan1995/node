...
function drawCurves(ptss, color) {
	...
	drawKnots([].concat(pts['inter_x'], pts['inter_y'], pts['turnPts']), 255);
}
...
function mouseReleased() {
	if (isMoveCurve) {
		...
		var bezier = genericBezier(sample(drawnPtsPartial));
		console.log(findMaxima(bezier));
		console.log(findMinima(bezier));
		bezier['inter_x'] = findInterceptX(bezier);
		bezier['inter_y'] = findInterceptY(bezier);
		bezier['turnPts'] = findTurningPts(bezier);
		bezier['bindSym'] = [];
		drawnPoints.push(bezier);
